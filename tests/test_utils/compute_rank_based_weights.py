...
from nichejepa.utils import compute_rank_based_weights
...
class TestComputeRankBasedWeights(unittest.TestCase):
    def test_basic_case(self):
        ...
        computed_weights = compute_rank_based_weights(tokens)
    ...
    def test_complex_case(self):
        tokens = torch.tensor([[1, 5, 0, 0],
                               ...
                               [1, 5, 7, 6],
                               [1, 111, 3, 0]])
        expected_weights = torch.tensor([[0.6667, 0.3333, 0.0000, 0.0000],
                                         [0.0000, 0.0000, 0.0000, 0.0000],
                                         [0.4000, 0.3000, 0.2000, 0.1000],
                                         [0.5000, 0.3333, 0.1667, 0.0000]])
        computed_weights = compute_rank_based_weights(tokens)
        print(computed_weights)
        torch.testing.assert_close(computed_weights,
                                   expected_weights,
