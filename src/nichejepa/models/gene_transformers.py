...
class GeneTransformerBaseEncoder(ABC, nn.Module):
    ...
    def return_multi_layer_emb() -> List[torch.Tensor]:
        """
        Encoder-specific logic for returning embeddings during inference.
        """
        pass
    ...
    def __init__(self,
        ...
        if pos_learnable:
            ...
            if self.n_special_tokens > 0:
                self.pos_embed.data[0, 0:self.n_special_tokens, :] = 0
        else:
...
class GeneTransformerRankPredictor(GeneTransformerBasePredictor):
    ...
    def forward(self,
            ...
            x_seg_embed = enc_seg_embed(segments)
...
class GeneTransformerCountPredictor(GeneTransformerBasePredictor):
    ...
    def forward(self,
            ...
            seg_embs = seg_embed(segments)

            # Add positional embeddings to tokens from context masks
            z_gene_embed = enc_token_embed(tokens)
            z_seg_embed = seg_embed(segments)
...
def init_gt_encoder(encoder_type: Literal['rank', 'count'],
               **encoder_kwargs
               ) -> GeneTransformerBaseEncoder:
...
def init_gt_predictor(predictor_type: Literal['rank', 'count'],
                 **predictor_kwargs
