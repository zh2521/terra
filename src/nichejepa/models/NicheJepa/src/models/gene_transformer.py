...
class TokenEmbed(nn.Module):
    ...
    """
    def __init__(self, vocab_size=224, hidden_size=16, padding_idx=0):
        super().__init__()
...
class TransformerPredictorGene(nn.Module):
    """ Transformer """
    def __init__(
        self,
        embed_dim=768,
    ...
    def forward(self, x, masks_enc, masks_pred):
        ...
        x[masks_pred[0]==1] = self.mask_token
        x_pos_embed = self.predictor_pos_embed.repeat(B, 1, 1)
        x += x_pos_embed
        masks = masks_enc[0] | masks_pred[0]
        for blk in self.predictor_blocks:
            x = blk(x,masks=masks.unsqueeze(1).repeat(1, masks.size(1), 1).unsqueeze(1))
...
class TransformerGene(nn.Module):
    """ Transformer """
    def __init__(
        self,
        vocab_size=963,
        ...
        **kwargs
        ...
        self.num_heads = num_heads
        # --
        self.patch_embed = TokenEmbed( vocab_size=vocab_size, hidden_size= embed_dim, padding_idx=0)
        #add here the lenght
...
def gene_predictor(**kwargs):
    model = TransformerPredictorGene(
        mlp_ratio=4, qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6),
        **kwargs)
    return model


def gene_tiny(seq_len=580, **kwargs):
    model = TransformerGene(
        seq_len=seq_len, embed_dim=192, depth=12, num_heads=3, mlp_ratio=4,
        qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    return model


def gene_small(seq_len=580, **kwargs):
    model = TransformerGene(
...
def gene_base(seq_len=580, **kwargs):
    model = TransformerGene(
...
def gene_large(seq_len=580, **kwargs):
    model = TransformerGene(
...
def gene_huge(seq_len=580, **kwargs):
    model = TransformerGene(
...
def gene_giant(seq_len=580, **kwargs):
    model = TransformerGene(
